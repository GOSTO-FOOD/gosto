...
const dishes = [
  {
    ...
    imagePath: "menu/signature/pizza-gosto.jpg",
  },
  {
    ...
    imagePath: "menu/signature/tacos-tramway.jpg",
  },
  {
    ...
    imagePath: "menu/signature/americain.jpg",
  },
];

function DishImage({ imagePath, gradient, pattern, accent, glow, children }: {
  imagePath: string;
  gradient: string;
  pattern: string;
  accent: string;
  ...
  const { url } = useStorageImage(imagePath);
  return (
    <div className="relative h-52 overflow-hidden" style={{ background: "rgba(0,0,0,0.8)" }}>
      {url ? (
        <>
          <img
            src={url}
          ...
          <div
            ...
            style={{ background: `linear-gradient(to top, ${glow} 0%, transparent 60%)` }}
          />
        </>
      ) : (
        <>
          ...
          <div className="absolute inset-0 bg-grid opacity-30" style={{ backgroundSize: "30px 30px" }} />
