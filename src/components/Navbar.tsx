...
import { Menu, X, UtensilsCrossed, Package, Utensils } from "lucide-react";
...
export default function Navbar() {
  ...
  const { count, openCart, openDineIn } = useCart();
    ...
    >
      <div className="container mx-auto px-4 md:px-8 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <button
            ...
            className="relative text-sm font-bold uppercase tracking-wider text-white/60 hover:text-white transition-colors group flex items-center gap-1.5"
          >
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button
            onClick={openDineIn}
            className="flex items-center gap-2 px-5 py-2.5 sharp font-display font-black text-xs uppercase tracking-wider transition-all hover:scale-105"
            style={{
              background: "rgba(57,255,20,0.08)",
              border: "1px solid rgba(57,255,20,0.4)",
              color: "#39FF14",
              boxShadow: "0 0 14px rgba(57,255,20,0.1)",
            }}
          >
            <Utensils size={13} />
            En Salle
      ...
      {mobileOpen && (
        <div className="md:hidden glass-dark border-t border-white/8 px-4 py-6 flex flex-col gap-2">
          ...
          <button
            onClick={() => { setMobileOpen(false); navigate("/track-order"); }}
            className="w-full text-left px-4 py-3.5 sharp font-bold text-white uppercase tracking-wider text-sm hover:bg-white/5 hover:text-[#FF7A00] transition-all flex items-center gap-3"
          >
            <Package size={16} className="text-[#FF7A00]/60" />
            Suivi de commande
          </button>
          <button
            onClick={() => { setMobileOpen(false); openDineIn(); }}
            className="w-full text-left px-4 py-3.5 sharp font-black uppercase tracking-wider text-sm transition-all flex items-center gap-3"
            style={{
          ...
          >
            <Utensils size={16} style={{ color: "#39FF14" }} />
            Commander sur Table
          </button>
        </div>
      )}
